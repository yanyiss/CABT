...
void InteractiveViewerWidget::draw_selected_edge()
{
	...
	//	int k = geo_test->poly_k;
	//	float* control_point = new float[3*3];
	//	for (int i = 0; i < geo_test->ControlPoint_List.size(); i++)
	//	{
	//		for (int ci = 0; ci <= 2; ci++)
	//		{
	//			int id = geo_test->matrix_to_list[ci][0];
	//			control_point[3 * ci] = geo_test->ControlPoint_List[i][id][0];
	//			control_point[3 * ci+1] = geo_test->ControlPoint_List[i][id][1];
	//			control_point[3 * ci+2] = geo_test->ControlPoint_List[i][id][2];
	//		}

	//		glMap1f(GL_MAP1_VERTEX_3, 0.0, 1.0, 3, 3, control_point);
	//		glColor3f(1.0, 0, 0);

	//		glBegin(GL_POINTS);
	//		for (int ci = 0; ci < 3; ci++)
	//		{
	//			glVertex3fv(&control_point[ci*3]);
	//		}
	//		glEnd();
...
{
	if (MeshViewer->haveFunction_AnisotropicMeshing())
	{
		...
		if (!fileName.isEmpty())
		{
			am->load_ref_mesh(fileName.toLocal8Bit());
		}
	}
}
*/

void InteractiveViewerWidget::Anisotropic()
{
	aniso_remesh = new anisotropic_meshing_interface();

	aniso_remesh->SetMesh(&mesh);
	std::string fi = "D:\\sig2020\\Surface_Framework\\Data\\curvedCubeRef.obj";
	aniso_remesh->load_ref_mesh(fi.data());

//	double t_e_l = MeshParam->get_target_edge_length_AM();
//	double s_r = MeshParam->get_sample_ratio_AM();
	double t_e_l = 0.2;
	double s_r = 2.0;
	aniso_remesh->do_remeshing(t_e_l, s_r);

	updateGL();
}

void InteractiveViewerWidget::Load_Ref_Mesh(const char* filename)
{
	aniso_remesh = new anisotropic_meshing_interface();
	aniso_remesh->SetMesh(&mesh);
	aniso_remesh->load_ref_mesh(filename);
}

void InteractiveViewerWidget::do_Remeshing(double target_l, double ratio_)
{
	aniso_remesh->do_remeshing(target_l, ratio_);
	updateGL();
}
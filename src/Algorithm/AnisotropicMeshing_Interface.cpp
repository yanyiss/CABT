...
void anisotropic_meshing_interface::load_ref_mesh(const char* filename)
{
	if(ref_mesh_) delete ref_mesh_;
	ref_mesh_ = new Mesh();
	bool ok = OpenMesh::IO::read_mesh( *ref_mesh_, filename);
	if(ok)
	{
...
#if 0
			double kk = 0.5*(std::abs(k1)+std::abs(k2));
			k1 = kk; k2 = kk;
#endif
	...
	printf("------------------------------------------------------------\n");
	printf("Finish Loading Ref Mesh!\n");
}
...
void anisotropic_meshing_interface::compute_src_mesh_ave_anisotropic_edge_length()
{
	...
	/*FILE* f_metric = fopen("metric.txtx", "w");
	for (Mesh::VertexIter v_it = ref_mesh_->vertices_begin(); v_it != ref_mesh_->vertices_end(); ++v_it)
	{
		OpenMesh::Vec6d& h0 = ref_mesh_->data(v_it).get_Hessian();
		fprintf(f_metric, "%.19f %.19f %.19f %.19f %.19f %.19f\n", h0[0], h0[1], h0[2], h0[3], h0[4], h0[5]);
	}
	fclose(f_metric);*/
}
...
void anisotropic_meshing_interface::do_remeshing(double ref_edge_len /* = 1.0 */, double a /* = 1.5 */)
{
	...
	for(unsigned ii =0;ii<10;++ii)
	{
		...
		std::cout << "one iter is OK " << std::endl;
	}
...
bool anisotropic_meshing_interface::reposition_LCOT(double step_length)
{
	...
	for(Mesh::VertexIter v_it = mesh_->vertices_begin(); v_it != mesh_->vertices_end(); ++v_it)
	{
		...
		/*if (std::abs(p[1] - 0.945313) + std::abs(p[2]) < 1e-5 )
		{
			new_pos[vertex_id] = p;
			continue;
		}*/
